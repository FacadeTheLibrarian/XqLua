...
たとえば
```csharp
_reactiveProperty.Subscribe(value => Debug.Log(value)).AddTo(_disposables);
```
としたとき、**Subscribeを呼んだ時点で`Debug.Log(value)`が実行**されます。

この挙動はUniRxでも同等で、この発火を受け取りたくない場合は`.Skip(1)`のOperatorを付けることで最初の発火を無視することができます。      
なお、`.Skip(1)`の例のように、**Operatorは消費される**ので注意してください。
